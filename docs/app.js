...
async function init() {
    try {
        ...
        document.querySelector('main').innerHTML =
            '<p style="text-align:center;padding:3rem;color:var(--text-muted)">No data available yet. Run <code>python export_to_web.py</code> to generate data.</p>';
    }
}
...
function renderFixtures(stageFilter = 'all', playedOnly = false) {
    ...
    if (stageFilter === 'group') {
        fixtures = fixtures.filter(f => f.stage.length === 1); // single letter = group
    ...
    container.innerHTML = fixtures.map(f => {
        ...
        return `
            <div class="fixture-card ${played ? 'played' : ''}">
                <div class="fixture-home">${escapeHtml(f.home)}</div>
                <div class="fixture-score ${played ? '' : 'pending'}">${scoreText}</div>
                <div class="fixture-away">${escapeHtml(f.away)}</div>
    ...
    grid.innerHTML = sortedKeys.map(g => {
        ...
        const rows = teams.map((t, i) => `
            <tr class="${i < 2 ? 'qualified' : ''}">
                <td style="text-align:left">${escapeHtml(t.team)}</td>
                <td>${t.p}</td>
                <td>${t.w}</td>
                <td>${t.d}</td>
                <td>${t.l}</td>
